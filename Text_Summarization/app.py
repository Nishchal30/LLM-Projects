...
st.set_page_config(page_title="Welcome to URL summarie app!", page_icon="youtube")
st.title("Welcome to text summarization app!")
st.subheader("Summarize URL")

st.sidebar.title("Settings")
groq_api_key = st.sidebar.text_input("Enter your GROQ api key:", type="password")
model = st.sidebar.selectbox("Select the open source model to run the Chatbot", ["llama3-70b-8192", "gemma2-9b-it", "llama3-8b-8192", "mistral-saba-24b", "qwen-qwq-32b", "deepseek-r1-distill-llama-70b"])
...
# def get_youtube_transcript_any_language(video_url):
#     try:
    ...
    if not groq_api_key.strip() or not url.strip():
        ...
        try:
            with st.spinner("Waiting..."):
                # Inside your Streamlit summarize button handler
                if "youtube.com" in url:
                    loader = YoutubeLoader.from_youtube_url(youtube_url=url)
                    st.info("Detected YouTube URL. Attempting to fetch transcript.")
                    # transcript_text = get_youtube_transcript_any_language(url)
                    # print(transcript_text)
                    # if transcript_text:
                    #     data = [Document(page_content=transcript_text)]
                    # else:
                    #     st.error("Could not extract transcript from YouTube video. Make sure the video has captions.")
                    #     st.stop()
                else:
